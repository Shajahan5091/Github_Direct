...
orders_summary as (
    select
        orders.customer_id,
        coalesce(sum(orders.total_amount), 0) as total_spending_last_365_days
...
customer_tier_calculation as (
    select
        ...
        case
            when coalesce(orders_summary.total_spending_last_365_days, 0) >= 1000 then 'Gold'
            when coalesce(orders_summary.total_spending_last_365_days, 0) >= 500 then 'Silver'
            else 'Bronze'
