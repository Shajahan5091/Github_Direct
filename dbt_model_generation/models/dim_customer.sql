{#
------------------------------------------------------------------------
MODEL NAME    : dim_customer
TARGET TABLE  : BSL_MA.DWH_MA.DIM_CUSTOMER
SOURCE TABLES : BSL_RAW.DWH_RAW.CUSTOMERS, BSL_RAW.DWH_RAW.ORDERS
DESCRIPTION   : Customer dimension table with customer details and derived 
                customer tier based on spending patterns in last 365 days
PREREQUISITES : Source tables CUSTOMERS and ORDERS must be available
PARAMETER     : None
AUTHOUR       : AI Generated by Shajahan - 2025-11-27
------------------------------------------------------------------------
#}

{{ config(
    materialized = 'view',
    alias = 'dim_customer'
) }}

WITH customers_source AS (
    SELECT
        customer_id,
        first_name, -- auto corrected using semantic file
        last_name, -- auto corrected using semantic file
        email,
        created_at,
        inserted_at
    FROM {{ source('dwh_raw', 'customers') }}
),

orders_source AS (
    SELECT
        customer_id,
        total_amount,
        order_date
    FROM {{ source('dwh_raw', 'orders') }}
),

customer_spending AS (
    SELECT
        orders.customer_id,
        SUM(orders.total_amount) AS total_spending_365_days
    FROM orders_source AS orders
    WHERE orders.order_date >= DATEADD('day', -365, CURRENT_DATE())
    GROUP BY orders.customer_id
),

transformed AS (
    SELECT
        customers.customer_id,
        UPPER(TRIM(customers.first_name)) AS first_name,
        UPPER(TRIM(customers.last_name)) AS last_name,
        CASE 
            WHEN customers.email IS NOT NULL 
                AND REGEXP_LIKE(customers.email, '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$')
            THEN LOWER(customers.email)
            ELSE NULL -- auto corrected using semantic file
        END AS email,
        DATE(customers.created_at) AS customer_since_date,
        CASE 
            WHEN COALESCE(spending.total_spending_365_days, 0) >= 1000 THEN 'Gold'
            WHEN COALESCE(spending.total_spending_365_days, 0) >= 500 THEN 'Silver'
            ELSE 'Bronze'
        END AS customer_tier,
        COALESCE(customers.inserted_at, CURRENT_DATE()) AS last_updated -- auto corrected using semantic file
    FROM customers_source AS customers
    LEFT JOIN customer_spending AS spending
        ON customers.customer_id = spending.customer_id
)

SELECT * FROM transformed