{#
------------------------------------------------------------------------
MODEL NAME    : dim_product
TARGET TABLE  : BSL_MA.DWH_MA.DIM_PRODUCT
SOURCE TABLES : BSL_RAW.DWH_RAW.PRODUCTS
DESCRIPTION   : Dimension table for products containing product details,
                pricing information, and price bands for analytical reporting
PREREQUISITES : Source table PRODUCTS must exist with valid data
PARAMETER     : None
AUTHOUR       : AI Generated by Shajahan - 2025-11-27
------------------------------------------------------------------------
#}

{{ config(
    materialized = 'table',
    alias = 'dim_product'
) }}

WITH source_data AS (
    SELECT
        product_id,
        product_name,
        price,
        inserted_at
    FROM {{ source('dwh_raw', 'products') }}
),

transformed AS (
    SELECT
        product_id,
        INITCAP(TRIM(product_name)) AS product_name, -- auto corrected using semantic file
        price,
        CASE 
            WHEN price < 50 THEN 'Low'
            WHEN price >= 50 AND price <= 199.99 THEN 'Mid'
            WHEN price >= 200 AND price <= 499.99 THEN 'High'
            WHEN price >= 500 THEN 'Premium'
        END AS price_band,
        inserted_at AS last_updated
    FROM source_data
)

SELECT * FROM transformed